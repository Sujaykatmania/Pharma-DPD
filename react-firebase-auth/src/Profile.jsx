...
const Profile = () => {
    ...
    const handleAddItem = async (field, value, setValue) => {
        ...
        try {
            const result = await validateMedicalTerm({ term: value });
    ...
    const renderListAndForm = (title, list, value, setValue, fieldName, placeholder) => (
        <div>
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleAddItem(fieldName, value, setValue); }} className="flex gap-2">
                ...
                />
                <button type="submit" disabled={isValidating} className="px-4 py-2 bg-gradient-to-br from-blue-600/80 to-blue-900/80 hover:from-blue-600 hover:to-blue-900 text-white font-bold rounded-md shadow-lg border border-white/30 transition-all duration-200 hover:shadow-xl active:scale-95 disabled:opacity-50">
